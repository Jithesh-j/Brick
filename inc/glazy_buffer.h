...
namespace glazy {
	...
	namespace compat {
		...
		// macOS does not support OpenGL 4.5, and so does not have glNamedBufferData.
	...
	public:
		...
		void set_data(T& data, GLenum usage) {
			...
			compat::named_buffer_data(id, sizeof(T), (void*) & data, usage);
		...
		void set_data(std::vector<T> const & data, GLenum usage) {
			safety::entry_guard("Buffer::set_data");
			compat::named_buffer_data(id, sizeof(T) * data.size(), (void*) data.data(), usage);
			safety::exit_guard("Buffer::set_data");
		}
		void set_data(std::vector<T> const && data, GLenum usage) {
			set_data(data, usage);
