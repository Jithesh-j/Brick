...
//
//void print_error(std::string context) {
//    if(!triggered) {
//        GLenum err = glGetError();
//        if (err != GL_NO_ERROR) {
//            std::cout << "ERROR CODE " << err << " at " << context << std::endl;
//            std::cout.flush();
//            triggered = true;
//        }
//    }
...
int PrintGLErrors(const char *title) {
	...
	for (;;) {
		...
		if (n == GL_NO_ERROR)
			break;
        snprintf(buf+strlen(buf), sizeof(buf)-strlen(buf), "%s%s", !nErrors++? "" : ", ", ErrorString(n));
...
int EnableVertexAttribute(int program, const char *name) {
	GLint id = glGetAttribLocation(program, name);
//    print_error("glGetAttrib..");
	if (id < 0 && squawk)
		printf("cant find attribute %s\n", name);

	if (id >= 0) {
        glEnableVertexAttribArray(id);
//        print_error("glEnableVert");
    }
	return id;
}
